...
- **heatmap over the binary frame lane** — `mlx90640.html` and `pressure_array.html`,
  both on `web/shared/heatmap.js` (`/sensors/{id}/frames` WS; the pressure viewer
  re-grids from each sweep header and shows missing-cell diagnostics).
...
(Effector-control viewers are still TODO.)
