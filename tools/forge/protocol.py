"""protocol.py — versioned MCU↔node link frame codec (forge "wire" v1).

This is the SINGLE source of truth for how an MCU serializes a vector of raw
channel samples onto its uplink. The node-side array sensor decodes with the
functions here; the firmware's transport module encodes the identical layout
(mcu/<family>/modules/transport_serial). Keep the two in lockstep — the
